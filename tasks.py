...
NAME = os.getenv("IMAGE_NAME", "netbox-example-plugin")
PWD = os.getcwd()

COMPOSE_FILE = "development/docker-compose.yml"
BUILD_NAME = "netbox_example_plugin"
...
def makemigrations(context, name="", netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    if name:
        context.run(
            f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run netbox python manage.py makemigrations --name {name} -v 3",
            env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        )
    else:
        context.run(
            f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run netbox python manage.py makemigrations -v 3",
            env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        )
